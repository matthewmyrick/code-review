...
Appa is a Tauri 2 desktop app. All logic lives in a Rust workspace; the
React frontend is a thin, cache-fed view layer.

```
crates/appa-core     domain types + unified-diff parser (no I/O)
crates/appa-github   read-only GitHub REST client, pluggable auth
crates/appa-cache    SQLite: GitHub cache + local review store
crates/appa-agents   headless agent runners + JSONL run logs
...
- Agent processes stream stdout/stderr through unbounded channels into a
  single event loop per run (`appa-agents::runner`), which serializes to
...
3. Backend emits `appa://sync {key, phase}` around every sync so the UI
...
2. `start_agent_review` builds a prompt: Appa context block (identity,
   the appa_comment contract, "never post to GitHub") + user
   ...
   - custom: `sh -c <command>`
3. Runner streams events; `{"type":"appa_comment"}` lines (stdout or the
