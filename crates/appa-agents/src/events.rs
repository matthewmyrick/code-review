...
/// Pull `appa_comment` lines out of a structured runner envelope.
///
...
mod tests {
    ...
    fn extracts_comments_from_assistant_text() {
        let comment = r#"{"type":"appa_comment","path":"a.rs","line":3,"body":"off by one"}"#;
    ...
    fn extracts_comments_from_result_text() {
        let comment = r#"{"type":"appa_comment","path":"b.ts","line":9,"body":"nit"}"#;
        let envelope = serde_json::json!({
            "type": "result",
            "subtype": "success",
    ...
    fn payload_roundtrips_and_dedupes() {
        let c =
            parse_comment(r#"{"type":"appa_comment","path":"a.rs","line":7,"body":"x"}"#).unwrap();
